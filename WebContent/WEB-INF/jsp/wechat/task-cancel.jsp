...
<% String cancelDate=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(Calendar.getInstance().getTime()); %>
...
<body>
	<form action="" method="post" onsubmit="return check">
		...
		<input type="hidden" name="canceledBy" value="${userAccount}"/>
		<input type="hidden" name="canceledDate" value="<%=cancelDate%>"/>
		<input type="hidden" name="lastEditedBy" value="${userAccount}"/>
		<input type="hidden" name="lastEditedDate" value="<%=cancelDate%>"/>	
		<!--  	按钮 -->
		<div class="btn1"> 
			<button type="submit"  data-loading="稍候..." class="btn-save">取消</button>
